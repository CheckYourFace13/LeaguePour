...
export async function GET(request: Request) {
  ...
  try {
    ...
    // statuses - see src/lib/outreach-email.ts). Plain Prisma filter now that vsStatus is
    // correctly declared as String in schema.prisma (it was previously mistyped as the
    // OutreachStatus enum, which doesn't match the live TEXT column and threw on any comparison
    // - see the vsStatus field comment in schema.prisma for the full history).
